...
// [Spec.Now] rather than a clock, [Spec.Quota] rather than a rate, a burst and
// an override function. Each saves this package a decision that has already
// been made.
//
// What is absent is as deliberate: there is no base URL, HTTP client, request
// timeout or response-size cap here, because this package makes no requests.
// Those four belong to the HTTP client that owns the engine.
type Spec struct {
	...
	Quota func(userID string) config.Quota
