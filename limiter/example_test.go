...
func exampleLimiter(srv *httptest.Server, tweak func(*pace.Config)) *pace.Limiter {
	cfg := pace.Config{BaseURL: srv.URL, Rate: limit.PerMinute(60), Burst: 10}
	if tweak != nil {
		tweak(&cfg)
	}
...
func ExampleLimiter_Client() {
	...
	lim := exampleLimiter(srv, func(c *pace.Config) { c.Burst = 1; c.Rate = limit.PerMinute(6) })
...
func ExampleLimiter_ReloadQuotas() {
	tiers := map[string]limit.Quota{"trial-42": {Rate: limit.PerMinute(6), Burst: 1}}
