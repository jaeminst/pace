...
func TestShutdownWaitsForInFlightRequest(t *testing.T) {
	...
	lim, err := client.New(config.Config{BaseURL: srv.URL, Rate: config.PerMinute(600), Burst: 10})
	if err != nil {
		t.Fatal(err)
	}
...
func TestNoStoreAccessAfterClose(t *testing.T) {
	...
	limiter.WaitGCLoop(lim.Limiter())
...
func TestClose_StoreCloseError(t *testing.T) {
	...
	limiter.SetLimiterStore(pool.Limiter(), &mockCloseErrStore{})
	pool.Close()
}
...
func TestShutdown_GracefulFinish(t *testing.T) {
	...
	for range 3 {
		go func() {
			defer wg.Done()
			_, _ = pool.Client("u").Get(context.Background(), "/")
		}()
	}
	wg.Wait()
...
func TestShutdown_ForcedOnTimeout(t *testing.T) {
	...
	if err := pool.Client("u").Wait(context.Background()); err != nil {
		t.Fatal(err)
	}
	...
	go func() { _ = pool.Client("u").Wait(context.Background()) }()
	<-waiting
...
func TestShutdown_RejectsNewRequests(t *testing.T) {
	...
	limiter.SetShuttingDownHook(pool.Limiter(), sync.OnceFunc(func() { close(flagged) }))
	...
	err = pool.Client("u2").Wait(context.Background())
