...
// skip the notification — nobody is listening, so no per-key list is built —
...
// Evict removes key from memory, persisting the current token state first
...
// lock this function took, and a slow store would stall every key who hashes
// to this shard.
//
...
func (r *Registry) Evict(ctx context.Context, key string) (bool, error) {
	now := r.cfg.Now()
	sh := r.shardFor(key)
...
// DropAll discards every key's in-memory state and counts the drop as
// evictions.
//
// It empties the shards rather than only reading them. A population count taken
// after shutdown must not report keys who no longer exist — the alternative is
// a snapshot that says "N keys" and "+N evictions" at the same time, which
// cannot both be true.
//
...
func (r *Registry) DropAll() {
	...
	if !notify {
		// Nobody is listening, so the per-key list was never built; count the
		// drop in one go instead.
		r.evictions.Add(max(0, dropped))
	}
}

// Sweep evicts idle keys in three phases, so that no store I/O ever happens
...
// let a fresh request for the same key load stale state, which the pending
...
func (r *Registry) Sweep() {
	...
	// A key who made a request in between keeps their live bucket; the value
	...
	for _, e := range expired {
		...
		sh.mu.Lock()
		if cur, ok := sh.keys[e.snap.Key]; ok && cur == e.u && cur.lastUsed.Load() == e.lastUsed {
			delete(sh.keys, e.snap.Key)
			sh.live.Add(-1)
...
// sweepInPlace evicts idle keys when there is no state to persist.
//
...
// keys, on the one path whose entire point is that it does almost nothing.
