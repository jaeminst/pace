...
`baseline-v0.5.0.txt` is the current reference. Compare against it with
...
`baseline-v0.1.0.txt` and `baseline-v0.3.0.txt` are kept for the historical
...
| `Sweep/store=sqlite` | `BenchmarkSweepWithStore` (root, black box) |
...
get. `BenchmarkSweepWithStore` in the root covers that path end to end, through a
real Limiter and its own batching flush.
