...
// Jobs is the durable queue's storage: the SQL that makes a job survive a
// restart and be sent once.
//
// It lives here rather than with the database handle because the guarantees are
// the statements. Claim is a single conditional UPDATE, and that one statement
// is why two workers racing for the same job cannot both win; Enqueue's
// resurrection guard and Release's ownership check are the same kind of thing.
// Reading them next to the poller that calls them is the point.
//
// The schema they read is still owned by github.com/jaeminst/pace/sqlite, which
// keeps one migration chain over one file. A column added there for a query
// here — pending_jobs.lease_until, or the (died_at, id) index Dead orders by —
// is a coupling the two packages have to keep by hand.
type Jobs struct{ db *sqlite.Store }

// NewJobs returns the queue's storage over an open database.
func NewJobs(db *sqlite.Store) *Jobs { return &Jobs{db: db} }
...
func (j *Jobs) Enqueue(ctx context.Context, job Job, now int64) error {
...
func (j *Jobs) Complete(ctx context.Context, id string, result Result, now int64) error {
	h, err := json.Marshal(result.Headers)
	if err != nil {
		return err
	}
	return j.db.Tx(ctx, func(tx *sql.Tx) error {
		...
		_, err := tx.ExecContext(ctx, `DELETE FROM pending_jobs WHERE id = ?`, id)
		return err
	})
}
...
func (j *Jobs) Claim(ctx context.Context, id, owner string, now, leaseUntil int64) (bool, error) {
...
// the same ownership.
func (j *Jobs) Release(ctx context.Context, id, owner string, now, nextAttemptAt int64, lastErr string) (bool, error) {
	res, err := j.db.Exec(ctx, `
		UPDATE pending_jobs
		   SET state = 'queued',
		       owner = '',
...
// why.
func (j *Jobs) Kill(ctx context.Context, id, reason string, now int64) (Job, bool, error) {
	var (
		job   Job
		found bool
	)
	err := j.db.Tx(ctx, func(tx *sql.Tx) error {
		...
		headers, err := json.Marshal(got.Headers)
		if err != nil {
			return err
		}
		if _, err := tx.ExecContext(ctx, `
			...
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
		`, got.ID, got.UserID, got.Method, got.Path, string(headers), got.Body, got.Attempts, reason, now); err != nil {
			return err
		}
...
func (j *Jobs) Dead(ctx context.Context, q DeadQuery) ([]Job, error) {
	...
	rows, err := j.db.Query(ctx, sql, args...)
	if err != nil {
		return nil, err
	}
...
func (j *Jobs) Get(ctx context.Context, id string) (*Result, bool, error) {
...
func (j *Jobs) Pending(ctx context.Context) ([]Job, error) {
...
func (j *Jobs) Due(ctx context.Context, now int64, limit int) ([]Job, error) {
	rows, err := j.db.Query(ctx, `
		SELECT id, user_id, method, path, headers, body, state, attempts
		FROM pending_jobs
...
func (j *Jobs) ClaimN(ctx context.Context, id, owner string, now, leaseUntil int64) (bool, int, error) {
	ok, err := j.Claim(ctx, id, owner, now, leaseUntil)
	if err != nil || !ok {
		return false, 0, err
	}
	var attempts int
	if err := j.db.QueryRow(ctx,
...
func (j *Jobs) PurgeResults(ctx context.Context, cutoff int64, chunk int) (int64, error) {
	var total int64
	for {
		res, err := j.db.Exec(ctx, `
