...
// Client is a rate-limited HTTP caller bound to one key identity. Obtain one
...
// rather than on one key's view of it.
type Client struct {
	key  string
	pool *Pool
}

// Key returns the identity this Client is bound to.
...
// so a Request that is built and then abandoned costs the key nothing.
func (c *Client) Request() *Request {
	return newRequest(c.pool, c.key)
}
...
// either, which is why it takes a context. A key's first request may load
...
func (c *Client) Allow(ctx context.Context) bool {
	return c.pool.lim.Allow(ctx, c.key)
}

// Wait blocks until a token is available for this key, ctx is done, or the
...
func (c *Client) Wait(ctx context.Context) error {
	return c.pool.lim.Wait(ctx, c.key)
}
...
// Tokens returns the tokens currently available to this key, and whether the
...
func (c *Client) Tokens() (float64, bool) {
	return c.pool.lim.Tokens(c.key)
}

// Evict removes this key from memory, first persisting their token state if a
...
func (c *Client) Evict(ctx context.Context) (bool, error) {
	return c.pool.lim.Evict(ctx, c.key)
}

// Quota returns the rate and burst in force for this key.
//
...
// [Pool.ReloadQuotas]. Otherwise it is what the key would be given on its next
// request. Unlike [Client.Tokens] it always has an answer, because a quota is
// configuration rather than state.
func (c *Client) Quota() bucket.Quota {
	return c.pool.lim.Quota(c.key)
}
...
func (c *Client) Reserve(ctx context.Context) *limiter.Reservation {
	return c.pool.lim.Reserve(ctx, c.key)
}

// ReloadQuota re-reads this key's quota and applies it to their live bucket,
// keeping the tokens they have already accrued. It reports whether the key had
// a bucket in memory at all.
//
// This is [Pool.ReloadQuotas] for one key, and it is O(1) where that walks
// every shard. It is not [Client.Evict], which also drops the accrued tokens and
// writes to the store on the way out.
func (c *Client) ReloadQuota() bool { return c.pool.lim.ReloadQuota(c.key) }
